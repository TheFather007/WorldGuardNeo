package dev.dizzy.worldguardneo.commands;
...
import dev.dizzy.worldguardneo.region.*;
...
public final class WGCommands {
    ...
    private static int claimRegion(CommandSourceStack src, String id, WorldGuardNeo mod) throws CommandSyntaxException {
        ...
        return mod.worldEdit().toProtectedRegion(p, finalId).map(region -> {
            ...
            net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(
                    new dev.dizzy.worldguardneo.api.events.RegionModifyEvent(
                            p.serverLevel(), finalRegion,
                            dev.dizzy.worldguardneo.api.events.RegionModifyEvent.ModifyType.CREATED, p));
    ...
    private static int redefineRegion(CommandSourceStack src, String id, WorldGuardNeo mod) throws CommandSyntaxException {
        ...
        return mgr.get(id).map(existing -> mod.worldEdit().toProtectedRegion(p, id).map(newRegion -> {
            ...
            if (existing instanceof dev.dizzy.worldguardneo.region.GlobalRegion) {
            ...
            net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(
                    new dev.dizzy.worldguardneo.api.events.RegionModifyEvent(
                            ...
                            dev.dizzy.worldguardneo.api.events.RegionModifyEvent.ModifyType.UPDATED, p));
    ...
    private static int removeRegion(CommandSourceStack src, String id, WorldGuardNeo mod) throws CommandSyntaxException {
        ...
        if (r.get() instanceof dev.dizzy.worldguardneo.region.GlobalRegion) {
        ...
        net.neoforged.neoforge.common.NeoForge.EVENT_BUS.post(
                new dev.dizzy.worldguardneo.api.events.RegionModifyEvent(
                        p.serverLevel(), r.get(),
                        dev.dizzy.worldguardneo.api.events.RegionModifyEvent.ModifyType.DELETED, p));
        mgr.remove(id);
        mod.regions().save(p.serverLevel());
        var bm = dev.dizzy.worldguardneo.integrations.BluemapIntegration.get();
    ...
    private static void printRegion(CommandSourceStack src, WorldGuardNeo mod, ProtectedRegion r) {
        ...
        src.sendSuccess(() -> Component.literal(mod.i18n().format("msg.info.owners",
                "list", r.ownersView().stream()
                        .map(u -> dev.dizzy.worldguardneo.util.UuidResolver.nameOf(server, u))
    ...
    private static String primaryOwnerName(net.minecraft.server.MinecraftServer server, ProtectedRegion r) {
        ...
        return dev.dizzy.worldguardneo.util.UuidResolver.nameOf(server, first);
    }
    ...
    private static int listPlayerRegions(CommandSourceStack src, String playerArg, WorldGuardNeo mod)
        ...
        var resolved = dev.dizzy.worldguardneo.util.UuidResolver.resolve(server, playerArg);
        if (resolved.isEmpty()) {
            err(src, mod, "msg.list.player.notfound", "player", playerArg);
            return 0;
        }
        final java.util.UUID uid = resolved.get();
        String resolvedName = dev.dizzy.worldguardneo.util.UuidResolver.nameOf(server, uid);
    ...
    private static int teleportToRegion(CommandSourceStack src, String id, WorldGuardNeo mod) throws CommandSyntaxException {
        ...
        if (r instanceof dev.dizzy.worldguardneo.region.GlobalRegion) {
            err(src, mod, "msg.region.global-select", "id", id);
            return 0;
        }
        Vec3 mn = r.minimumBound(), mx = r.maximumBound();
     ...
     * {@link dev.dizzy.worldguardneo.util.UuidResolver}: it may be a raw UUID, an online
    ...
    private static int changeMembership(CommandContext<CommandSourceStack> c,
        ...
        var uuidOpt = dev.dizzy.worldguardneo.util.UuidResolver.resolve(self.getServer(), who);
        if (uuidOpt.isEmpty()) {
        ...
        if (!changed) {
            err(c.getSource(), mod, add ? "msg.member.already" : "msg.member.notpresent",
                    "player", dev.dizzy.worldguardneo.util.UuidResolver.nameOf(self.getServer(), target),
