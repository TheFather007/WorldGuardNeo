...
public final class LuckPermsResolver implements PermissionResolver {
    ...
    public boolean has(ServerPlayer player, String node) {
        ...
        // Standard "LuckPerms overrides, OP is the default" model — what players expect:
        ...
        //               (OpResolver). This is what makes the mod work out of the box: OP-0 nodes
        //               (claim/info/list/...) work for everyone and admin nodes work for operators
        //               without an admin having to grant every worldguardneo node in LP first.
        //               (Previously UNDEFINED hard-denied AND ignored OP, which hid the entire /rg
        //               command tree from admins who manage perms via groups.) region.bypass stays
        //               OP-5 in OpResolver, so it is still never granted by OP alone — explicit LP only.
