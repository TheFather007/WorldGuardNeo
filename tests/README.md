...
| `GeometryCrossCheckTest` | **Differential PIP:** validates `PolygonalRegion.contains` (crossing-number) against an independent winding-number implementation over dense grids of many random rectangles/L-shapes (~30k assertions; even vertices sampled at odd coords → no boundary ambiguity). |
...
> `JsonStorageTest` and `ConfigTest` need the mojang-logging + slf4j (and night-config) jars on the
> classpath; `run.sh` locates them in the Gradle cache automatically.

### Not covered here — enforcement / in-game behavior
