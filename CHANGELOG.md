...
**ИСПРАВЛЕНО (по логу сервера): команды `/rg` и `/region` «не существовали» при LuckPerms.**
Лог подтвердил: мод грузится нормально (WorldEdit и LuckPerms найдены, `Ready.`), но команд нет.
Причина — в `LuckPermsResolver`: при результате LuckPerms **UNDEFINED** (право не задано в LP)
возвращался **deny**, причём OP-уровень полностью игнорировался. У админа, управляющего правами
через группы без явных нод `worldguardneo.*`, ВСЕ `.requires()` подкоманд `/rg` становились
ложными → Brigadier скрывал корень команды целиком → «команды не существует».

- Теперь модель стандартная «**LuckPerms переопределяет, OP — значение по умолчанию**»:
  LP `TRUE` → разрешено; LP `FALSE` → запрещено (LP может отозвать доступ даже у OP);
  LP `UNDEFINED` → откат на per-node OP-уровень (`OpResolver`). Так мод работает «из коробки»:
  оператор получает дефолтные ноды (claim/info/list = OP 0 — у всех; админ-ноды — по OP-уровню),
  не требуя выдавать каждую ноду в LP вручную. `region.bypass` остаётся OP 5 (по OP не выдаётся —
  только явный грант в LP).
- Добавлена строка в лог при регистрации: `[WorldGuardNeo] Registered /region and /rg commands.`
  — для быстрой диагностики, что дерево команд встало.

---

## build 16 — 2026-06-13

**ИСПРАВЛЕНО (мод не загружался → не работала ни одна команда):** в `neoforge.mods.toml`
зависимость `worldedit` была `type="required"`. На сервере без установленного WorldEdit
NeoForge **полностью отключал мод** — поэтому не регистрировалась ни одна команда (`/rg`,
`/region`) ни в игре, ни в консоли, причём молча (только строкой в логе про отсутствие
обязательной зависимости).

- `worldedit` теперь **`type="optional"`**. Мод загружается без WorldEdit; работают все команды,
  защита и флаги. WorldEdit нужен только для **создания** регионов (`/rg claim`, `/rg redefine`),
  которые читают его выделение.
- `WorldEditAdapter.isAvailable()` + понятные сообщения: `/rg claim`/`redefine` без WorldEdit
  теперь отвечают «WorldEdit не установлен — он нужен для создания регионов» (ключ
  `msg.selection.no-worldedit`, en+ru), а не загадочное «сделайте выделение».
- README/RU: WorldEdit перенесён из «обязательно» в «опционально» с пояснением.

Это устраняет самый частый симптом «команды не работают» у тех, кто не ставил WorldEdit.

---

## build 15 — 2026-06-13

Персональные тесты для каждого флага + расширенный отчёт в файл.

**`PerFlagReport`** (`tests/`): для КАЖДОГО из 84 флагов запускает индивидуальную батарею —
контракт (тип, дефолт, узел права, ключ описания, подсказка, round-trip реестра), резолюция
(вайлдернесс=дефолт; deny для владельца/участника/чужого/null; allow), приоритет (high-deny бьёт
low-allow), наследование родителя, групповой фильтр (-g OWNERS), build-access для build-флагов, а
для value-флагов — parse + resolveValue + групповое ограничение + JSON round-trip. По окончании
пишет подробный **отчёт в `tests/FLAG_REPORT.txt`**: на каждый флаг — заголовок с метаданными
(type/default/perm/enforced/hint) и список `[PASS]/[FAIL]` по пунктам, плюс машинно-читаемый
футер `SUMMARY`/`RESULT`. Итог: **1178 per-flag проверок, ALL GREEN**.

**Раннер** `tests/run.sh` теперь вызывает `PerFlagReport` и генерирует отчёт. Суммарно ~2422
JVM-проверки в 6 наборах + страж покрытия, все зелёные.

---

## build 14 — 2026-06-13

Внутриигровая GameTest-батарея — тесты реального слоя enforcement (события/миксины), который
JVM-наборы достать не могут.

**`WGNGameTests`** (`dev.thefather007.worldguardneo.gametest`, `@GameTestHolder`): каждый тест
создаёт регион на тестовой платформе и выполняет настоящее действие в живом `ServerLevel`,
проверяя итог защиты:
- **block-break / build:** mock-игрок ломает блок — запрет чужому (членство), разрешение владельцу,
  явный `block-break allow` открывает чужому, явный `deny` бьёт членство владельца, вайлдернесс
  разрешён.
- **interact / use:** ПКМ по люку — запрет чужому, разрешение участнику, `use allow` открывает чужому.
- **random-tick миксины:** `leaf-decay` (deny сохраняет листву / контроль — опадает), `ice-melt`,
  `snow-melt` (deny сохраняет блок при форсированных random-tick).
- **события сущностей:** `lightning` (deny — молния не появляется / allow — появляется),
  `pvp` (deny — урон не проходит).
- **движок:** живая резолюция `getApplicable`/`testBuildAccess` в реальном `ServerLevel`,
  членство.

**Инфраструктура:** пустой шаблон-структура `worldguardneo:platform` (SNBT); gradle-ран
`runGameTestServer` с включённым namespace gametest. Запуск также через `/test runall` (в окружении
должен быть WorldEdit — обязательная зависимость; создание регионов в тестах программное, без WE).

Сборка зелёная; JVM-наборы (~1240 проверок) по-прежнему проходят.
