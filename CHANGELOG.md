...
## build 16 (v1.3) — 2026-06-15

Крупное расширение возможностей: экономика клеймов, истечение неактивных клеймов, командные
флаги входа/выхода, инкрементальное хранилище и пара исправлений хранилища/производительности.

**Экономика клеймов** (по умолчанию выключена, `economy.enabled`). Встроенный баланс на игрока без
внешних модов: `/rg balance [игрок]`, `/rg pay <игрок> <сумма>`, `/rg sell <id> <цена>`,
`/rg unsell <id>`, `/rg buy <id>`, админское `/rg eco <set|give|take> <игрок> <сумма>`. Покупка
переводит средства продавцу, делает покупателя владельцем, уважает лимит регионов. Данные —
`economy.json`. Узлы прав `worldguardneo.economy.use` / `.admin`.
