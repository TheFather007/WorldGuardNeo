...
**Переопределение по командам.** Чтобы поменять OP-уровень *отдельной* команды, не трогая тиры, добавьте её в таблицу `[command-op-levels]` в `config.toml` (узел → уровень). Префикс `worldguardneo.` необязателен; ключи с точкой берутся в кавычки; уровни `0`–`5`. Это переопределяет дефолты admin/mod и перечитывается по `/rg reload`:

```toml
[command-op-levels]
"region.teleport" = 0   # /rg teleport доступен всем
"region.claim"    = 2   # для claim нужен op 2
"reload"          = 4
```
